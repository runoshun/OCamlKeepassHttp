...
module KeepassProviderTests(Backends : Backends.Interface) = struct
  ...
  let provider_ref = ref (MyResult.Error "provider is not set.")

  let init thunk =
    Provider.with_provider config (fun provider_or_error ->
      match provider_or_error with
      | MyResult.Ok provider ->
          provider_ref := (MyResult.Ok provider);
          thunk ()
      | MyResult.Error e ->
          provider_ref := (MyResult.Error e))
  ...
  let test_client_config () =
    ...
    | MyResult.Ok provider ->
        ...
        assert_equal (MyResult.Ok key) client_key;
        match no_client_key with
        | MyResult.Ok _ -> assert_failure "expected not found, but client_key found."
        | MyResult.Error _ -> ()
